...
const professionalResultLabel = { '1B':'一垒安打', '2B':'二垒安打', '3B':'三垒安打', HR:'本垒打', BB:'四坏保送', HBP:'触身球', K:'三振', OUT:'出局' };

function professionalPlayer(game, side, playerId) {
  return game[side].players?.find((player) => player.id === playerId);
}

function professionalPanel(game) {
  if (game.mode !== 'professional' || !game.professional || !game.professionalStats) return '';
  const offense = game.half === 'top' ? 'away' : 'home';
  const defense = offense === 'away' ? 'home' : 'away';
  const battingLineup = game.professional.lineups[offense];
  const pitchingLineup = game.professional.lineups[defense];
  const batter = professionalPlayer(game, offense, battingLineup.order[battingLineup.currentBatterIndex]);
  const pitcher = professionalPlayer(game, defense, pitchingLineup.pitcherId);
  const table = (side, label) => {
    const stats = game.professionalStats[side];
    const activePitcher = game.professional.lineups[side].pitcherId;
    const pitchers = stats.pitching.filter((item) => item.player.id === activePitcher || item.outs || item.h || item.r || item.bb || item.so);
    return `<div class="pro-stat-block"><div class="pro-stat-title"><strong>${esc(label)}</strong><span>R ${stats.totals.runs} · H ${stats.totals.hits}</span></div><div class="pro-table-wrap"><table class="pro-table"><thead><tr><th>球员</th><th>AB</th><th>R</th><th>H</th><th>RBI</th><th>BB</th><th>SO</th></tr></thead><tbody>${stats.batting.map((item)=>`<tr><td>${esc(item.player.number ? `${item.player.number} ${item.player.name}` : item.player.name)}</td><td>${item.ab}</td><td>${item.r}</td><td>${item.h}</td><td>${item.rbi}</td><td>${item.bb}</td><td>${item.so}</td></tr>`).join('')}</tbody></table></div><div class="pro-pitching-title">投手</div><div class="pro-pitching-wrap"><table class="pro-table"><thead><tr><th>球员</th><th>IP</th><th>H</th><th>R</th><th>ER</th><th>BB</th><th>SO</th></tr></thead><tbody>${pitchers.map((item)=>`<tr><td>${esc(item.player.number ? `${item.player.number} ${item.player.name}` : item.player.name)}</td><td>${item.ip}</td><td>${item.h}</td><td>${item.r}</td><td>${item.er}</td><td>${item.bb}</td><td>${item.so}</td></tr>`).join('')}</tbody></table></div></div>`;
  };
  return `<article class="card professional-card"><div class="card-head"><strong>专业模式 · 逐打席记录</strong><button class="btn ghost" data-action="professional-setup">编辑名单</button></div><div class="card-body"><div class="pro-matchup"><div><small>当前打者</small><strong>${esc(batter?.name || '未设置')}</strong><span>${esc(offense === 'away' ? game.away.shortName : game.home.shortName)}</span></div><div><small>当前投手</small><strong>${esc(pitcher?.name || '未设置')}</strong><span>${esc(defense === 'away' ? game.away.shortName : game.home.shortName)}</span></div></div><div class="pro-action-grid">${Object.entries(professionalResultLabel).map(([result,label])=>`<button class="pro-action ${result}" data-action="professional-pa" data-result="${result}">${label}</button>`).join('')}</div><p class="help">每次选择一个打席结果，系统会自动更新打序、垒包、比分和打击/投手统计。复杂跑垒、换投和守备事件将在下一阶段加入。</p></div></article><article class="card professional-card"><div class="card-head"><strong>实时 Box Score</strong><small>专业模式数据预览</small></div><div class="card-body pro-stat-grid">${table('away', game.away.shortName)}${table('home', game.home.shortName)}</div></article>`;
}
...
function render() {
  ...
  app.innerHTML = `<div class="shell">
    ...
    <div class="layout"><aside class="sidebar"><div class="sidebar-title"><strong>比赛历史</strong><span>${state.games.length} 场</span></div><div class="game-list">
      ${state.games.map((item)=>`<div class="game-entry"><button class="game-item ${item.id===game.id?'active':''}" data-action="activate-game" data-id="${item.id}"><span class="game-name">${esc(item.title)}</span><span class="game-meta"><span><i class="status-dot ${item.status}"></i>${item.status==='finished'?'已结束':'进行中'}</span><span>${dateLabel(item.updatedAt)}</span></span></button><button class="game-delete" data-action="delete-game" data-id="${item.id}" title="删除此场比赛" ${state.games.length<=1?'disabled':''}>删除</button></div>`).join('')}
      ...
      <main class="content"><div class="page-head"><div><h1>${esc(game.title)}</h1><p>${esc(game.away.fullName)} vs ${esc(game.home.fullName)} · 计划 ${game.scheduledInnings} 局 · 实际赛程不受限制</p></div><span class="live-state ${game.syncPaused?'paused':''}">${game.syncPaused?'● 直播画面已锁定':'● 正在实时同步'}</span></div>
        <div class="grid">${game.syncPaused?`<div class="sync-banner"><span>当前修改尚未显示在直播比分牌上。</span><span class="row"><button class="btn ghost" data-action="discard">放弃修改</button><button class="btn warn" data-action="publish">发布到直播</button></span></div>`:''}
          ...
          ${professionalPanel(game)}</section>
...
function newGameModal() {
  modal({ title:'新建比赛', confirmText:'创建比赛', body:`<div class="form-grid"><div class="field"><label>比赛名称</label><input id="new-title" value="新比赛 ${new Date().toLocaleDateString('zh-CN')}" /></div><div class="field"><label>记录模式</label><select id="new-mode"><option value="basic">基础模式：比分与直播</option><option value="professional">专业模式：球员名单、逐打席与 Box Score</option></select></div><div class="form-two"><div class="field"><label>客队简称</label><input id="new-away" value="AWAY" maxlength="12" /></div><div class="field"><label>主队简称</label><input id="new-home" value="HOME" maxlength="12" /></div></div><div class="field"><label>计划局数（仅作提示）</label><input id="new-innings" type="number" min="1" max="99" value="9" /></div><p class="help">专业模式会自动创建两队各 9 名初始球员，之后可在专业记录卡中编辑名单。</p></div>`, onConfirm:async(root)=>{ const title=root.querySelector('#new-title').value.trim(); const away=root.querySelector('#new-away').value.trim(); const home=root.querySelector('#new-home').value.trim(); state=await window.baseballAPI.newGame({title,mode:root.querySelector('#new-mode').value,scheduledInnings:Number(root.querySelector('#new-innings').value),away:{fullName:away,shortName:away},home:{fullName:home,shortName:home}}); render(); } });
}

function professionalSetupModal() {
  const game = activeGame();
  const lines = (side) => game[side].players.map((player) => `${player.number},${player.name},${player.position}`).join('\n');
  const parse = (value, existing) => value.split(/\r?\n/).map((line) => line.trim()).filter(Boolean).map((line, index) => {
    const [number='', name='', position=''] = line.split(/[，,]/).map((part) => part.trim());
    return { id: existing[index]?.id, number: number || String(index + 1), name: name || `球员${index + 1}`, position };
  }).slice(0, 30);
  modal({ title:'编辑专业模式名单', confirmText:'保存名单', body:`<div class="form-grid"><p class="help">每行按“号码,姓名,位置”填写，例如：10,张三,SS。前 9 名球员将自动作为初始打序，位置为 P 的球员会作为初始投手。</p><div class="field"><label>${esc(game.away.shortName)} 球员名单</label><textarea id="pro-away-roster" rows="10">${esc(lines('away'))}</textarea></div><div class="field"><label>${esc(game.home.shortName)} 球员名单</label><textarea id="pro-home-roster" rows="10">${esc(lines('home'))}</textarea></div></div>`, onConfirm:async(root)=>{const away=parse(root.querySelector('#pro-away-roster').value, game.away.players);const home=parse(root.querySelector('#pro-home-roster').value, game.home.players);if(!away.length||!home.length){toast('两支球队都至少需要一名球员');return false;}await dispatch({type:'SET_PROFESSIONAL_ROSTER',side:'away',players:away});await dispatch({type:'SET_PROFESSIONAL_ROSTER',side:'home',players:home});} });
}
...
app.addEventListener('click', async (event) => {
  ...
  if(action==='professional-setup')return professionalSetupModal();
  if(action==='professional-pa'){const result=el.dataset.result;const eventAction={type:'RECORD_PRO_PLATE_APPEARANCE',result,label:professionalResultLabel[result]||'专业打席记录'};if(result==='HR')return confirmAction('确认本垒打','系统将自动计算垒上跑者和打者得分，并更新专业统计。',eventAction,'确认本垒打');if((result==='K'||result==='OUT')&&game.outs>=2)return confirmAction('确认第三个出局','确认后将结束当前半局，并清空 BSO 与垒上跑者。',eventAction,'确认记录');return dispatch(eventAction);}
  if(action==='edit-team')return editTeamModal(el.dataset.side); if(action==='edit-title')return editTitleModal();
  if(action==='run'){if(game.mode==='professional'){toast('专业模式请通过打席记录产生得分，修正功能将在后续版本加入。');return;}return dispatch({type:'ADJUST_RUN',side:el.dataset.side,delta:Number(el.dataset.delta)});}
  if(action==='toggle-base'||action==='clear-bases'){if(game.mode==='professional'){toast('专业模式的垒上跑者由打席记录自动维护。');return;}return action==='toggle-base'?dispatch({type:'SET_BASE',index:Number(el.dataset.index),value:!game.bases[Number(el.dataset.index)]}):dispatch({type:'CLEAR_BASES'});}
  if(action==='ball'){if(game.mode==='professional'){if(game.balls>=3)return confirmAction('确认四坏保送','确认后会写入专业打席记录，并自动处理进垒和统计。',{type:'RECORD_PRO_PLATE_APPEARANCE',result:'BB',label:'四坏保送'},'执行保送');return dispatch({type:'BALL'});}if(game.balls>=3)return confirmAction('确认四坏保送','这是第四个坏球。确认后打者上一垒，并自动处理被迫进垒；满垒时将增加一分。',{type:'WALK'},'执行保送');return dispatch({type:'BALL'});}
  if(action==='walk')return game.mode==='professional'?confirmAction('确认四坏保送','确认后会写入专业打席记录，并自动处理进垒和统计。',{type:'RECORD_PRO_PLATE_APPEARANCE',result:'BB',label:'四坏保送'},'执行保送'):confirmAction('确认四坏保送','系统将自动处理被迫进垒，并清空球数。',{type:'WALK'},'执行保送');
  if(action==='strike'){if(game.mode==='professional'){if(game.strikes>=2)return confirmAction('确认三振出局',game.outs>=2?'这是第三个好球且已有两出局，确认后将自动结束当前半局。':'这是第三个好球，确认后会写入专业打席统计。',{type:'RECORD_PRO_PLATE_APPEARANCE',result:'K',label:'三振'},'确认三振');return dispatch({type:'STRIKE'});}if(game.strikes>=2)return confirmAction('确认三振出局',game.outs>=2?'这是第三个好球且已有两出局，确认后将自动结束当前半局。':'这是第三个好球，确认后出局数加一并清空球数。',{type:'STRIKEOUT'},'确认三振');return dispatch({type:'STRIKE'});}
  if(action==='foul')return dispatch({type:'FOUL'});
  if(action==='out'){const proAction={type:'RECORD_PRO_PLATE_APPEARANCE',result:'OUT',label:'出局'};if(game.mode==='professional')return game.outs>=2?confirmAction('确认第三个出局','确认后会写入专业打席记录，并结束当前半局。',proAction,'结束半局'):dispatch(proAction);if(game.outs>=2)return confirmAction('确认第三个出局','确认后将清空 BSO 与垒包，并自动切换到下一半局。',{type:'OUT'},'结束半局');return dispatch({type:'OUT'});}
  if(action==='set-count')return dispatch({type:'SET_COUNT',field:el.dataset.field,value:Number(el.dataset.value)});
  if(action==='hit'){const bases=Number(el.dataset.bases);const names=['','一垒安打','二垒打','三垒打','本垒打'];if(game.mode==='professional'){const proAction={type:'RECORD_PRO_PLATE_APPEARANCE',result:['','1B','2B','3B','HR'][bases],label:names[bases]};return bases===4?confirmAction('确认本垒打','确认后会写入专业打席记录，并计算全部得分跑者。',proAction,'确认本垒打'):dispatch(proAction);}return confirmAction(`确认${names[bases]}`,bases===4?'所有垒上跑者与打者都将得分。':'系统将按固定垒数给出进垒结果。实际跑垒不同可在确认后直接修正垒包和比分。',{type:'APPLY_HIT',bases,label:names[bases]},'应用建议');}
  ...
  if(action==='delete-game'){const target=state.games.find((item)=>item.id===(el.dataset.id||game.id))||game;return modal({title:'删除比赛',danger:true,confirmText:'确认删除',body:`<p class="danger-text">将永久删除“${esc(target.title)}”及其操作记录。建议先导出备份。</p>`,onConfirm:async()=>{const result=await window.baseballAPI.deleteGame(target.id);if(!result.ok){toast(result.message);return false;}state=result.state;render();}});}
